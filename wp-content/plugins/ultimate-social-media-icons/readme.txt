=== Social Media Share Buttons and Social Share Icons (Ultimate Sharing) ===
...
- Step 5: Select from various other share options, e.g. make your social media icons 'float'
...
- **More design styles** to make your share icons look really cool & matching the design of your website
...
- **Position icons by anchor and margins**, i.e. you can define the margins where your share icons should be displayed (from top/bottom/left/right), and then have them floating, or stick, still visible when user scrolls down or not etc. 
...
= Share Icons don't show = 

Please ensure you actually placed the social share buttons either as social widget (in your widget area) or as floating icons under question 5). The Premium Plugin makes placing icons especially easy and also allows you to place sticky share icons on your site, define the placement of the share icons by margins and many other options, see https://www.ultimatelysocial.com/usm-premium/.

If only some share icons show, but not all, then please clear your cache, and check if you may have conflicting browser extensions (e.g. 'Disconnect'-app in Chrome). Also Ad-Blockers are known culprits, please switch them off temporarily to see if that is the reason.
...
If you are referring to specific share icons not showing in the plugin itself (e.g. you're looking for a Whatsapp share icon, but it doesnt exist) please note that our Premium Plugin has many more social media share icons, see https://www.ultimatelysocial.com/usm-premium/
...
= Changes don't get saved / Deleted plugin but share icons still show = 

Most likely you have the WP Cache plugin installed. Please de-activate and then re-activate it, then the share icons should display again.

= Links when clicking on share icons don't work = 

Please ensure you've entered the 'http://' at the beginning of the url (for *all* social networks). If the share icons are not clickable at all there is most likely an issue with your template. This is especially the case if you've given your social share buttons several features, which should show a pop-up (tooltip) when you move over the share icons. 

= I cannot upload custom share icons = 
...
= My Youtube share icon (direct follow) doesn't work = 
...
= Aligning the share icons (centered, left- or right-aligned) doesn't work = 

The alignment options under question 5 align the share icons with respect to each other, not where they appear on the page. Our new Premium Plugin is the best social sharing plugin on the market, allowing you to define also many other share icon alignments (e.g. within a widget, within shortcode etc.).  
...
= The pop-up shows although I only gave my share icon one function = 

The pop-up only disappears if you've given your share icons only a 'visit us'-function, otherwise (e.g. if you gave it 'Like' (on facebook) or 'Tweet' functions) a pop-up is still needed because the share buttons for those are coming directly from the social media sites (e.g. Facebook, Twitter) and we don't have any influence over their design.

= I selected to display share icons after every post but they don't show = 

The share icons usually do show, however not on your blog page, but on your single posts pages. The Premium plugin (https://www.ultimatelysocial.com/usm-premium/) also allows to display the share icons on your homepage.
...
- The more sharing- and invite- features you place on your site, the more external codes you load (i.e. from the social media sites; we just use their share code), therefore impacting loading speed. So to prevent this, give your share icons only 'Visit us'-functionality rather than sharing-functionalities.
...
...then most likely you're currently logged in with a business account on Facebook. Please logout, or switch to your personal account, and then try to share again.

= There are other issues when I activate the plugin or place the share icons = 
...
You can see this by activating the share 'counts' on the front end (under question 4 in the USM plugin, question 5 in the USM+ plugin).
...
You can change it in the Widget-area where you dropped the widget (with the share icons) on the sidebar. Please click on it (on the sidebar), it will open the menu where you can change the text.
...
Open the first question in the plugin ('1. Which icons do you want to show on your site?'), on the level of the email-icon you see a link on the right hand side. Please click it to remove the credit link. Note that our premium plugin doesn’t place this link. 

= Can I use a shortcode to place the share icons? = 

Yes, use [DISPLAY_ULTIMATE_SOCIAL_ICONS] to show the share icons. You can place it into any editor.
...
= Can I get more options for the share icons next to posts? = 
...
To get the email-icon in the same design style you picked, activate it, then on the front-end, rightclick on the share icon, and save it as picture. Upload that picture as custom share icon.

= Can I also display the share icons vertically? = 
...
= Can I deactivate the share icons on mobile? = 
...
5. Here you can animate your share icons (automatic shuffling, mouse-over effects etc.), to make visitors of your site aware that they can share, follow & like your site

6. You can choose to display counts next to your share icons (e.g. number of Twitter-followers)
