...
if (have_comments()) {
	...
	if (get_comments_number() > get_option('comments_per_page')) { ?>
		<div class="pagination comments-pagination">
			<?php paginate_comments_links(array('prev_text' => __('&laquo;', 'mh-newsdesk-lite'), 'next_text' => __('&raquo;', 'mh-newsdesk-lite'))); ?>
		</div><?php
	}
	...
	if (!comments_open()) { ?>
		<p class="no-comments"><?php _e('Comments are closed.', 'mh-newsdesk-lite'); ?></p><?php
	}
} else {
	if (comments_open()) {
...
if (comments_open()) {
	$custom_args = array(
    	'title_reply' => __('Leave a comment', 'mh-newsdesk-lite'),
		'comment_notes_before' => '<p class="comment-notes">' . __('Your email address will not be published.', 'mh-newsdesk-lite') . '</p>',
		'comment_notes_after'  => '',
		'comment_field' => '<p class="comment-form-comment"><label for="comment">' . __('Comment', 'mh-newsdesk-lite') . '</label><br/><textarea id="comment" name="comment" cols="45" rows="5" aria-required="true"></textarea></p>');
